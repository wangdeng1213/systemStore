...
<body>
	<div class="container-fluid">
		<form:form cssClass="form-horizontal" modelAttribute="methodModel"
			method="PUT" action="/admin/resource/method/${id}/update">
			<fieldset>
				<legend>
					修改受保护方法信息
				</legend>
					...
					<div class="controls">
						<select id="rolesSelect" name="roleIds" multiple="multiple">
							<c:forEach items="${allRolesModel}" var="cur">
								<option value="${cur.id }" <c:forEach items="${methodModel.privilege.roles}" var="curRole"><c:if test="${cur.id == curRole.id }">selected</c:if></c:forEach>>${cur.name }</option>
							</c:forEach>
						</select>
					</div>
				</div>
