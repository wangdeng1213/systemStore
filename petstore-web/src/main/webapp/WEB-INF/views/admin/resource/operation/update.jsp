...
<body>
	<div class="container-fluid">
		<form:form cssClass="form-horizontal" modelAttribute="operationModel"
			method="PUT" action="/admin/resource/operation/${id}/update">
			<fieldset>
				<legend>
					修改页面操作信息
				</legend>
					...
					<div class="controls">
						<select id="rolesSelect" name="roleIds" multiple="multiple">
							<c:forEach items="${allRolesModel}" var="cur">
								<option value="${cur.id }" <c:forEach items="${operationModel.privilege.roles}" var="curRole"><c:if test="${cur.id == curRole.id }">selected</c:if></c:forEach>>${cur.name }</option>
							</c:forEach>
						</select>
					</div>
				</div>
