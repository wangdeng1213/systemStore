...
<body>
	<div class="container-fluid">
		<form:form cssClass="form-horizontal" modelAttribute="pageElementModel"
			method="PUT" action="/admin/resource/pageElement/${id}/update">
			<fieldset>
				<legend>
					修改受保护页面元素信息
				</legend>
					...
					<div class="controls">
						<select id="rolesSelect" name="roleIds" multiple="multiple">
							<c:forEach items="${allRolesModel}" var="cur">
								<option value="${cur.id }" <c:forEach items="${pageElementModel.privilege.roles}" var="curRole"><c:if test="${cur.id == curRole.id }">selected</c:if></c:forEach>>${cur.name }</option>
							</c:forEach>
						</select>
					</div>
				</div>
