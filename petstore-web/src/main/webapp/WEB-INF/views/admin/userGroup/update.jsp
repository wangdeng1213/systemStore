...
<body>
	<div class="container-fluid">
		<form:form cssClass="form-horizontal" modelAttribute="userGroupModel"
			method="PUT" action="/admin/userGroup/${id}/update">
			<fieldset>
				<legend>
					修改用户组信息
				</legend>
					...
					<div class="controls">
						<select id="rolesSelect" name="roleIds" multiple="multiple">
							<c:forEach items="${allRolesModel}" var="cur">
								<option value="${cur.id }" <c:forEach items="${userGroupModel.roles}" var="curRole"><c:if test="${cur.id == curRole.id }">selected</c:if></c:forEach>>${cur.name }</option>
							</c:forEach>
						</select>
					</div>
				</div>
